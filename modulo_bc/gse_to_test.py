                        ...
                        if len(rrq_data) >= 4:
                            ...
                            if rrq_opcode == TFTP_OPCODE.RRQ.value:
                                ...
                                if requested == header_filename:
                                    ...
                                    print(f"[✓] Socket de transferência criado na porta {transfer_port} (TID)")
                                        ...
                                        with open(requested, 'rb') as f:
                                            ...
                                            while offset < len(file_data):
                                                ...
                                                while retries < MAX_RETRIES:
                                                    ...
                                                    transfer_sock.sendto(data_pkt, (rrq_addr[0], rrq_addr[1]))
                                                    try:
                                                        # Aguarda ACK no socket efêmero
                                                        ack_pkt, _ = transfer_sock.recvfrom(516)
                                                        ack_opcode = struct.unpack("!H", ack_pkt[0:2])[0]
                                                        ack_block = struct.unpack("!H", ack_pkt[2:4])[0]
                                                        if ack_opcode == TFTP_OPCODE.ACK.value and ack_block == block_num:
                                                            break
                                                        retries += 1
                                                    except socket.timeout:
                                                        retries += 1
                                                ...
                                                if offset >= len(file_data):
                                                    ...
                                                    transfer_sock.sendto(hash_packet, (rrq_addr[0], rrq_addr[1]))
